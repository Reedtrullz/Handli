...
| G3 Data truth | partial | [product-truth contract](../contracts/v1-product-truth.md), [official/history ADR](../adr/0001-official-offer-vs-historical-price.md), and [foundation verification](../evidence/v1/foundation-2026-07-16.md) | Candidate-release proof of append-only eligible evidence, explicit coverage states, scope/expiry enforcement, and official/history separation through scheduled worker, API, and UI. |
| G4 Planner correctness | partial | Phase 1 property/unit tests and the v1 domain contracts | Independent-oracle and corpus proof for whole-package quantities, multibuy, maximum three stores, deterministic travel-aware non-dominated frontier, endpoints, and partial coverage. |
...
| G7 Offline shopping | not-started | v1 design and work plan | Production PWA trip completed offline on iOS and Android physical devices, with immutable trip snapshot and API-cache exclusions verified. |
...
| G9 Security, privacy, and legal | partial | Protected Cloudflare Access preview, server-only credential boundary, source registry | Accepted threat model, Norwegian privacy notice, data-flow inventory, license/third-party exclusions, rights/marks review, headers, abuse controls, dependency/license/secret scans, correction and security contacts. |
| G10 Operations | partial | Forward migrations, health-gated protected deployment, rollback assets, real-PostgreSQL package tests, and the [001-to-008 foundation proof](../evidence/v1/foundation-2026-07-16.md) | Candidate CI proof including checksum corruption and previous-image compatibility, encrypted off-host backup and isolated clean-host recovery including private blobs, worker leases/status, external alerts, immutable promoted image, and rollback drills. |
...
| G12 Public-good governance | partial | Public-good goal in the v1 plan and ranking constraints in the product-truth contract | Published code license, mission, funding/sponsorship disclosure, reproducible ranking policy, correction process, contributor governance, and operator/data-controller identity. |
