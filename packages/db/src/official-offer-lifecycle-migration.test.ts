...
describe("official-offer publication runtime migration", () => {
  it("precedes the publication-health hardening migration", async () => {
    const files = (await readdir(migrationsDirectory))
      ...
      .sort();
    expect(files.at(-10)).toBe("024_operations_runtime_boundary.sql");
    expect(files.at(-9)).toBe("025_private_review_evidence_renderer.sql");
    expect(files.at(-8)).toBe("026_official_offer_publication_runtime.sql");
    expect(files.at(-7)).toBe("027_official_offer_publication_health.sql");
    expect(files.at(-6)).toBe("028_private_review_image_evidence_only.sql");
    expect(files.at(-5)).toBe("029_kassalapp_source_approval.sql");
    expect(files.at(-4)).toBe("030_catalog_measure_optional.sql");
    expect(files.at(-3)).toBe("031_supported_chain_expansion.sql");
    expect(files.at(-2)).toBe("032_seed_national_scope.sql");
  });
