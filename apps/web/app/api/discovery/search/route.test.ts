...
describe("GET /api/discovery/search", () => {
  it("browses when no query is supplied", async () => {
    let called = false;
    ...
    const response = await createDiscoverySearchHandler(() => service)(new Request("https://handleplan.no/api/discovery/search"));
  ...
  it("rejects short, oversized, duplicate, and unexpected parameters", async () => {
    const handler = createDiscoverySearchHandler(() => ({ browse: async () => { throw new Error("unused"); }, search: async () => { throw new Error("unused"); } }));
  ...
  it.each([
    ...
    const response = await createDiscoverySearchHandler(() => ({ browse: async () => { throw error; }, search: async () => { throw error; } }))(request("melk"));
