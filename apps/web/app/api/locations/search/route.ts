...
const DEFAULT_LOCATION_SEARCH_TIMEOUT_MS = 5_000;
const MAX_BODY_BYTES = 2 * 1024;
const MAX_RESPONSE_BYTES = 8 * 1024;

type ServiceProvider = () =>
  | LocationSearchServiceContract
  | Promise<LocationSearchServiceContract>;
...
function validatedResponse(value: unknown): Response {
  ...
  if (!parsed.success) return errorResponse("LOCATION_SEARCH_UNAVAILABLE", 503);
  let body: string;
  try {
    body = JSON.stringify(parsed.data);
  } catch {
    return errorResponse("LOCATION_SEARCH_UNAVAILABLE", 503);
  }
...
export function createLocationSearchHandler(
  ...
  const timeoutMs = resolveRequestTimeoutMs(options, DEFAULT_LOCATION_SEARCH_TIMEOUT_MS);
  return async function POST(request: Request): Promise<Response> {
    ...
    try {
      ...
      const parsed = locationSearchRequestSchema.safeParse(body.value);
      ...
      try {
        ...
        const result = await awaitWithinRequest(
          () => service.search(parsed.data, lifetime.signal),
          lifetime.signal,
        );
        ...
        if (error instanceof LocationSearchServiceError) {
...
export async function POST(request: Request): Promise<Response> {
  ...
  const { getProductionLocationSearchService } = await import(
    "../../../../lib/server/travel/location-search-service"
  );
  const container = getServerContainer();
  return createLocationSearchHandler(
