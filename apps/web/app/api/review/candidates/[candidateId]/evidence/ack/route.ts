...
function privateProofResponse(value: unknown): Response {
  const response = privateBoundedJson(value, MAX_RESPONSE_BYTES);
...
export function createReviewEvidenceAckHandler(
  ...
  return async function POST(request: Request, candidateIdInput: string): Promise<Response> {
    ...
    try {
      ...
      const parsed = reviewEvidenceAckRequestV1Schema.safeParse(body.value);
      ...
      const result = await awaitWithinRequest(
        () => service.acknowledge(parsed.data, principal, lifetime.signal),
        lifetime.signal,
      );
      return privateProofResponse(result);
    } catch (error) {
...
const handler = createReviewEvidenceAckHandler(async () => {
