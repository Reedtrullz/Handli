...
const DEFAULT_TRAVEL_PLAN_TIMEOUT_MS = 10_000;
const MAX_BODY_BYTES = 64 * 1024;
const MAX_RESPONSE_BYTES = 160 * 1024;

type ServiceProvider = () =>
  | TravelPlanCoordinatorContract
...
function bestEffortCancelBody(request: Request): void {
  try {
    ...
    // Cleanup cannot replace a sanitized public response.
  }
}
...
function planningErrorResponse(error: unknown): Response {
  if (error instanceof TravelPlanCoordinatorError) {
    return error.code === "INVALID_REQUEST"
      ? errorResponse("INVALID_REQUEST", 400)
      : errorResponse("INVALID_SERVICE_RESPONSE", 503);
  }
...
function validatedResponse(
  request: Parameters<typeof travelPlanApiResponseSchemaFor>[0],
  value: unknown,
): Response {
  const parsed = travelPlanApiResponseSchemaFor(request).safeParse(value);
...
export function createTravelPlansHandler(
  getService: ServiceProvider,
  options: ControlledPublicApiRouteOptions = {},
) {
  const timeoutMs = resolveRequestTimeoutMs(options, DEFAULT_TRAVEL_PLAN_TIMEOUT_MS);
  return async function POST(request: Request): Promise<Response> {
    ...
    try {
      ...
      const parsed = travelPlanApiRequestSchema.safeParse(body.value);
      ...
      try {
        ...
        return validatedResponse(parsed.data, result);
      } catch (error) {
        const aborted = requestAbortResponse(lifetime);
        if (aborted !== undefined) return aborted;
...
export async function POST(request: Request): Promise<Response> {
  ...
  const { getProductionTravelPlanCoordinator } = await import(
    "../../../../lib/server/travel/travel-plan-service"
  );
  const container = getServerContainer();
  return createTravelPlansHandler(
