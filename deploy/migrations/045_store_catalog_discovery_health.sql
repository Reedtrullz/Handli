-- Let the store-scoped catalog+price walk advance source-health discovery
-- clocks. The trigger whitelist (last rewritten by migration 023) does not
-- know the new job kind, so the first production run with persisted evidence
-- would fail the discovery-progress check and roll back the deployment.
...
begin
  ...
  if worker_result.status in ('succeeded', 'partial')
    ...
    if worker_result.job_kind in ('catalog-refresh', 'official-offer-ingestion', 'store-catalog-price-refresh') then
