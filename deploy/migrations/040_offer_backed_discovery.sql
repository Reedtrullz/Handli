...
begin
  ...
  if position('p_product_ids' in v_body) <> 0 then
    raise exception 'public official-offer projection still references product IDs';
  end if;
  ...
  execute pg_catalog.format(E'
    create function public.public_offer_backed_discovery_rows_v1(
      ...
      set search_path = pg_catalog, pg_temp as %L', 'begin return query ' || v_offer_select || '; end;');
end;
